...
- To read in the Codebook associated with the Pre-Survey data

- To calculate mean average scores and average scores for 8 scales (AKA Research Question 1): 
1. PHQ-4 Anxiety & Depression
2. Ryff Psychological Wellbeing
3. Black Identity
4. LGBT Identity
5. Critical Consciousness
6. Collective/Community Self-Esteem
7. Radical Hope
8. Empowerment

This will contribute to Jacquelyn Chin's June 2025 dissertation defense. 
...
- AKA we want to remove fake data based on Q1 (which is the unique identifier for each participant)
...
- Employment
- Income
- Political Action (AKA Critical Action)
  - split into 5 questions 

First, we will read about how to [Recode Variables](https://www.anyamemensah.com/blog/recoding) in R. 

##Read in Codebook 

- This is the same Pre-Survey Codebook used in `rq2.Rmd`
...
codebook <- rbind(
  ...
  #get rid of the space in New Value
  ...
  mutate(CombinedValues = if_else(is.na(NewValue), Value, as.character(NewValue)),
         #note this is for the Pre-Survey data only
         Week = "Pre-Survey")
```
...
Note that one person used the open text field to enter data
...
Next, we will address Research Question 1, which makes hypotheses about 8 psychological scales. 

## PHQ-4 Anxiety & Depression

PHQ-4 Anxiety & Depression (ALL ITEMS in Q8.Q8_1, Q8_2, Q8_3, and Q8_4)
- Averaging the total of this measure's items for n people
...
Now, create a dataframe to visualize the results from the scale
...
This scale contains some questions that are reverse coded. The Codebook has been updated to reflect these reverse coded values. Before calculating mean averages, we will need to ensure the  values are coded correctly.   
...
pre_ryff_codebook <- left_join(x = pre_ryff, y = reverse_codes) %>% 
  #if_else() must work with characters
  mutate(
    NewValue = as.character(NewValue),
    Value = as.character(Value),
    #create a Corrected Value column that uses the NewValue if there is one, otherwise it uses the original Value
    CorrectedValue = if_else(is.na(NewValue), Value, as.character(NewValue)))

#reformat to calculate mean averages
pre_ryff <- pre_ryff_codebook %>% 
  select(
    -NewValue,
    -Value
  ) %>% 
...
n <- (ncol(pre_ryff) - 1)
print(n)
if(n >= ncol(pre_ryff))stop("Check n")

#calculate the mean score per participant
pre_ryff$average <- round(rowMeans(pre_ryff[grep('^Q9_', names(pre_ryff))]), digits = 2)

print(pre_ryff)

pre_ryff_mean_avg <- round(mean(pre_ryff$average), digits = 2)
print(pre_ryff_mean_avg)
```

```{r post_ryff}
post_ryff <- post_survey_clean %>% 
  select(
    Q1, 
    starts_with("Q9_")
  ) %>% 
...
#join the survey data to the codebook 
post_ryff_codebook <- left_join(x = post_ryff, y = reverse_codes) %>% 
  #if_else() must work with characters
  mutate(
    NewValue = as.character(NewValue),
    Value = as.character(Value),
...
n <- (ncol(post_ryff) - 1)
print(n)
if(n >= ncol(post_ryff))stop("Check n")

#calculate the mean score per participant
post_ryff$average <- round(rowMeans(post_ryff[grep('^Q9_', names(post_ryff))]), digits = 2)

print(post_ryff)

post_ryff_mean_avg <- round(mean(post_ryff$average), digits = 2)
print(post_ryff_mean_avg)
```

```{r follow_up_ryff}
follow_up_ryff <- follow_up_clean %>% 
  select(
    Q1, 
    starts_with("Q9_")
  ) %>% 
...
#join the survey data to the codebook 
follow_up_ryff_codebook <- left_join(x = follow_up_ryff, y = reverse_codes) %>% 
  #if_else() must work with characters
  mutate(
    NewValue = as.character(NewValue),
    Value = as.character(Value),
...
follow_up_ryff <- follow_up_ryff_codebook %>% 
  select(
    -NewValue,
    -Value
  ) %>% 
...
#how many questions are asked
n <- (ncol(follow_up_ryff) - 1)
print(n)
if(n >= ncol(follow_up_ryff))stop("Check n")

#calculate the mean score per participant
follow_up_ryff$average <- round(rowMeans(follow_up_ryff[grep('^Q9_', names(follow_up_ryff))]), digits = 2)

print(follow_up_ryff)

follow_up_ryff_mean_avg <- round(mean(follow_up_ryff$average), digits = 2)
print(follow_up_ryff_mean_avg)
```
...
#how many participants per survey ?
Participants <- c(nrow(pre_ryff), 
            nrow(post_ryff),
            nrow(follow_up_ryff)) 

#mean average score per week
MeanAverage <- c(pre_ryff_mean_avg, 
                 post_ryff_mean_avg, 
                 follow_up_ryff_mean_avg)

Average <- c(NA, 
             post_ryff$average,
             follow_up_ryff$average)
