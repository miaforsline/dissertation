...
codebook_anxiety <- read_excel(path = here(filepath, filename[1]), sheet = 2) %>%
  ...
  mutate("Reverse" = "NA",
         "New Value" = "NA")
codebook_consciousness <- read_excel(path = here(filepath, filename[1]), sheet = 3) %>% 
  mutate("Source Construct" = NA)
codebook_ryff <- read_excel(path = here(filepath, filename[1]), sheet = 4) %>% 
  mutate("Source Construct" = NA, 
         "Reverse" = NA,
         "New Value" = NA)
codebook_black <- read_excel(path = here(filepath, filename[1]), sheet = 5) %>% 
  mutate("Source Construct" = NA,
         "Reverse" = NA,
         "New Value" = NA)
codebook_lgbt <- read_excel(path = here(filepath, filename[1]), sheet = 6) %>% 
  mutate("Source Construct" = NA,
         "Reverse" = NA,
         "New Value" = NA)
codebook_action <- read_excel(path = here(filepath, filename[1]), sheet = 7) %>% 
  mutate("Source Construct" = NA,
         "Reverse" = NA,
         "New Value" = NA)
...
No numeric values in LGBT scale answers for Post-Survey
...
What numeric values correspond with the text answers ?
...
#calculate the mean score per participant
follow_up_community$average <- round(rowMeans(follow_up_community[grep('^Q21_', names(follow_up_community))]), digits = 2)
