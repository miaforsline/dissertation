...
pre_item1 <- pre_week2 %>% 
  ...
  summarize(avg = mean(Q18_1), n = n()) %>%
  ungroup %>%
  #adorn avg bottom row
  bind_rows(summarize(., Q1 = "Overall Avg", 
                         avg = sum(avg * n) / sum(n), 
                         n = sum(n))) %>%
  select(-n) %>% 
  rename(pre_survey = avg)

print(pre_item1)
```

```{r post_item1}
post_item1 <- post_week2 %>% 
  select(
    Q1, 
    Q18_1
  ) %>%
